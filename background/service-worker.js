...
async function runCode(source, languageId, stdin) {
  ...
  // Piston fallback: abort + check res.ok so a 4xx/5xx/html response doesn't
  // surface as a confusing JSON-parse SyntaxError (the Judge0 path above
  // already has both guards).
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), 12_000);
  let res;
  try {
    ...
    clearTimeout(t);
    return { output: '', error: 'Piston request failed: ' + (e?.message || e) };
  }
  clearTimeout(t);
  if (!res.ok) return { output: '', error: 'Piston HTTP ' + res.status };
  let d;
  try { d = await res.json(); }
  catch { return { output: '', error: 'Piston returned non-JSON response' }; }
...
async function injectMonacoIntoTab(tabId, nonce) {
  ...
  //
  // SECURITY: the MAIN world shares `window` with the page's own scripts, so
  // every bridge message in BOTH directions carries `nonce` and is validated
  // against it. A page script cannot guess the nonce (generated per session in
  // the isolated content script), so it cannot forge `cmd.*` requests nor read
  // or spoof `evt.*` / semantic-token replies.
  await chrome.scripting.executeScript({
    target: { tabId },
    world: 'MAIN',
    func: (loaderSrc, editorSrc, vsBase, workerSrc, cssSrc, nonce) => {
      // Guard against double injection
      if (window.__nzoi_editor_ready) {
        window.postMessage({ _nzoi: 'evt', n: nonce, evt: 'ready' }, '*');
        return;
      }
      if (window.__nzoi_editor_injecting) return;
      window.__nzoi_editor_injecting = true;

      // Pin the per-session nonce (passed in from the isolated content script)
      // so every listener/helper below can validate and echo it. `const NONCE`
      // is captured by the closures defined further down.
      const NONCE = nonce;
      ...
      function fail(msg) {
        ...
        window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'error', message: msg }, '*');
      }
...
`;
      ...
      window.addEventListener('message', e => {
        ...
        // Only accept semantic-token replies that carry our session nonce.
        if (e.data.n !== NONCE) return;
      ...
      function requestSemTokens(lang, value) {
        return new Promise(resolve => {
          ...
          window.postMessage({ _nzoiSem:'req', n: NONCE, lang, id, value }, '*');
          // Safety timeouts — never let a stuck request hang Monaco's highlighter
      ...
      window.addEventListener('message', function onMsg(e) {
        ...
        const d = e.data;
        // Reject commands that don't carry our per-session nonce — only the
        // isolated content script that requested this injection knows it, so a
        // page script cannot drive the editor (create/setValue/setMarkers/...).
        if (d.n !== NONCE) return;
        if (d.cmd === 'create') {
          ...
          _editor.onDidChangeModelContent(() => {
            window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'change', value: _editor.getValue() }, '*');
          });
          _editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.KeyS,
            () => window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'cmd.save' }, '*'));
          _editor.addCommand(window.monaco.KeyMod.Alt | window.monaco.KeyCode.KeyR,
            () => window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'cmd.run' }, '*'));
          _editor.addCommand(window.monaco.KeyMod.Alt | window.monaco.KeyCode.KeyS,
            () => window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'cmd.submit' }, '*'));
          window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'ready' }, '*');
        } else if (d.cmd === 'getValue') {
          window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'getValue.reply', value: _editor ? _editor.getValue() : '' }, '*');
      ...
      injectScript(loaderSrc, () => {
        ...
        injectScript(editorSrc, () => {
          ...
          window.require(['vs/editor/editor.main'], () => {
            ...
            window.postMessage({ _nzoi: 'evt', n: NONCE, evt: 'monaco-loaded' }, '*');
          }, (err) => fail('require failed: ' + (err && err.message || err)));
        }, fail);
      }, fail);
    },
    args: [loaderCode, editorCode, MONACO_BASE, workerCode, editorCss, nonce],
  });
  ...
  (async () => {
    try {
      ...
      switch (msg.type) {
        ...
        case 'inject:monaco':             await injectMonacoIntoTab(msg.tabId, msg.nonce); break;
