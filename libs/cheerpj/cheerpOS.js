...
{
	var runtimePath = self.cheerpjRuntimePath || loaderPath;
	var runtimeDownloader = runtimePath === loaderPath && useIframe ? IFrameProxyDownloader : DirectDownloader;
	...
	cheerpjFSMounts.push(new CheerpJDataFolder("/files/uploads/"));
	cheerpjFSMounts.push(new CheerpJIndexedDBFolder("/files/", /*cjMode*/true));
	cheerpjFSMounts.push(new CheerpJWebFolder("/lt/", runtimePath, null, true, runtimeDownloader));
	...
	if(useIframe && runtimePath === loaderPath)
...
function CheerpJIndexedDBFolder(mp, cjMode)
{
	CheerpJFolder.call(this, mp);
...
function idbEnsureDBConnection(mp, cb)
{
	if(mp.dbConnection == null){
		...
		// Upgrade indexDB version when using CheerpJ
		var openResult = mp.cjMode ? indexedDB.open("cjFS_"+mp.mountPoint, 2) : indexedDB.open("cjFS_"+mp.mountPoint);
		...
		openResult.onupgradeneeded = function(e)
		{
			...
			if (e.oldVersion == 0)
			{
				...
				if (mp.cjMode)
				{
					// Initialize the root dir and CheerpJ file dirs
					req = store.add({ type: "dir", contents:['/downloads', '/uploads'], uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0, inodeId: 1, nextInode:4 }, "");
					assert(req.readyState != "done");
					req = store.add({ type: "dir", contents:[], inodeId: 2, permType: CheerpJFileData.S_IFDIR | (511 & 01777), uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0}, "/downloads");
					assert(req.readyState != "done");
					req = store.add({ type: "dir", contents:[], inodeId: 3, permType: CheerpJFileData.S_IFDIR | (511 & 01777), uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0}, "/uploads");
					assert(req.readyState != "done");
				}
				else
				{
					// Initialize just the root dir
					req = store.add({ type: "dir", contents:[], uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0, inodeId: 1, nextInode:2 }, "");
					assert(req.readyState != "done");
				}
			...
			// Upgrade existing indexDB
			// TODO: Verify IndexDB version
			else
			{
				const tx = this.transaction;
				const store = tx.objectStore("files");
				const rootDir = store.get("");
				rootDir.onsuccess = function(e)
				{
					const entry = e.target.result;
					const downloadsInode = entry.nextInode++;
					const uploadsInode = entry.nextInode++;
					entry.contents.push("/downloads");
					entry.contents.push("/uploads");
					store.put(entry, "");
					store.put({ type: "dir", contents:[], inodeId: downloadsInode, permType: CheerpJFileData.S_IFDIR | (511 & 01777), uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0}, "/downloads");
					store.put({ type: "dir", contents:[], inodeId: uploadsInode, permType: CheerpJFileData.S_IFDIR | (511 & 01777), uid: 0, gid: 0, lastModified: (Date.now() / 1000)|0}, "/uploads");
				};
				tx.oncomplete = function()
				{
					this.oncomplete = null;
...
function cheerpOSWatchCapture(func, fileName)
{
	return function()
	{
		func(fileName);
...
function cheerpOSWatchNotify(fileName)
{
	for(var p in cosFileWatchPrefixes)
	{
		...
		{
			setTimeout(cheerpOSWatchCapture(cosFileWatchPrefixes[p], fileName), 0);
		}
	}
}
