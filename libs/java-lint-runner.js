...
// Per-session nonce passed by the parent (content.js) via the iframe URL
// fragment (libs/java-lint.html#n=<nonce>). The sandboxed iframe has an opaque
// origin, so origin-based postMessage checks cannot work here; instead every
// command the parent posts must carry this nonce, and every reply we send
// includes it so the parent can match it back to this iframe. A page script
// cannot guess the nonce, so it cannot drive javac/CheerpJ here nor forge lint
// results back into the editor.
const SESSION_NONCE = (() => {
  try {
    const m = /^[?#]n=([A-Za-z0-9_-]{8,128})/.exec(location.hash || location.search || '');
    if (m) return m[1];
  } catch {}
  // Fallback: a private nonce so the runner still self-defends if an older
  // caller forgot to pass one. The parent will then reject our replies, which
  // is the safe failure mode (no lint rather than spoofable lint).
...
function post(type, extra = {}) {
  parent.postMessage({ nzoiJavaLint: true, n: SESSION_NONCE, type, ...extra }, '*');
}
...
window.addEventListener('message', e => {
  ...
  // Reject any command that doesn't carry our per-session nonce. Only the
  // parent that created this iframe with #n=<nonce> can know it, so a page
  // script (different origin, no access to this iframe's location) cannot
  // drive javac/CheerpJ here.
  if (d.n !== SESSION_NONCE) return;
