...
// ── Custom model ID show/hide ─────────────────────────────────────────────────

function syncCustomInput(selectId, inputId) {
...
[].forEach(id => {
  const inp = id.replace('-model', '-model-custom');
  document.getElementById(id).addEventListener('change', () => syncCustomInput(id, inp));
});

// Resolve the actual model ID: if select is __custom__, use the text input.
function resolveModel(selectId, inputId) {
  const sel = document.getElementById(selectId);
  if (!sel) return '';
