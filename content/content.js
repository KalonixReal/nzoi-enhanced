...
init();`;
  ...
  // Per-session nonce shared with the Java-lint iframe. The iframe is sandboxed
  // (opaque origin), so we can't use origin checks on its postMessages; instead
  // we embed this nonce in the iframe URL fragment and the runner echoes it on
  // every reply. We also require e.source === the iframe's contentWindow, so a
  // page script (which can't obtain a === reference to our hidden iframe's
  // window, nor guess the nonce) cannot forge lint results into the editor.
  let _javaLintNonce = Math.random().toString(36).slice(2) + Date.now().toString(36);
  function postJavaLint(cmd, extra = {}) {
    if (!_javaLintFrame?.contentWindow) return;
    _javaLintFrame.contentWindow.postMessage({ nzoiJavaLintCmd: true, n: _javaLintNonce, cmd, ...extra }, '*');
  }
  function ensureJavaLintFrame() {
    if (_javaLintFrame) return;
    _javaLintFrame = document.createElement('iframe');
    // Nonce travels in the fragment so it never hits the network.
    _javaLintFrame.src = chrome.runtime.getURL('libs/java-lint.html') + '#n=' + encodeURIComponent(_javaLintNonce);
    ...
    _javaLintFrame.onload = () => { postJavaLint('init'); };
  ...
  window.addEventListener('message', e => {
    ...
    // Only accept messages from our own hidden Java-lint iframe, and only when
    // they carry this session's nonce. A page script cannot obtain a `===`
    // reference to the iframe's contentWindow nor guess the nonce, so it can't
    // push fake lint markers/errors into the editor.
    if (e.source !== _javaLintFrame?.contentWindow) return;
    if (d.n !== _javaLintNonce) return;
  ...
  //
  // SECURITY: this window is shared with the page's own JS (the isolated and
  // MAIN worlds both see the same window.postMessage stream), so a magic tag
  // alone is not enough — a page script could otherwise forge `cmd.submit`,
  // `setValue`, `setMarkers`, or semantic-token replies. We generate a
  // per-session nonce here, thread it through the executeScript(MAIN)
  // injection, and require it on every bridge message in both directions.
  // The page cannot guess it.

  const _edNonce = Math.random().toString(36).slice(2) + Date.now().toString(36);

  function edCmd(cmd, extra) {
    window.postMessage({ _nzoi: 'cmd', n: _edNonce, cmd, ...extra }, '*');
  }
  ...
  window.addEventListener('message', e => {
    ...
    const d = e.data;
    // Only trust events that carry our per-session nonce (set by our own MAIN
    // world injection). Prevents a page script from forging editor events.
    if (d.n !== _edNonce) return;
  ...
  window.addEventListener('message', e => {
    ...
    // Same per-session nonce as the _nzoi channel — only our own MAIN-world
    // Monaco injection can request semantic tokens.
    if (d.n !== _edNonce) return;
    if (d.lang === 'python') {
      requestPySemanticTokens(d.value, raw => {
        window.postMessage({ _nzoiSem:'res', n: _edNonce, id: d.id, data: encodeSemanticTokens(raw) }, '*');
      });
    } else if (d.lang === 'java') {
      const data = encodeSemanticTokens(computeJavaSemanticTokens(d.value));
      window.postMessage({ _nzoiSem:'res', n: _edNonce, id: d.id, data }, '*');
    } else {
      window.postMessage({ _nzoiSem:'res', n: _edNonce, id: d.id, data: [] }, '*');
    }
  });
  ...
  async function loadMonaco() {
    ...
    if (!_monacoInjected) {
      ...
      bg({ type: 'inject:monaco', tabId, nonce: _edNonce }).catch(e => {
  ...
  window.addEventListener('message', e => {
    // Exact origin match only — a substring check (`e.origin.includes(...)`)
    // was substring-attackable by any host whose name contains the clangd
    // domain (e.g. evil-clangd-in-browser-fork.pages.dev). The clangd iframe is
    // the only legitimate sender on this channel.
  ...
  // Sample cases are re-read on EVERY run instead of captured once at script
  // load: the statement is fetched/streamed after the page shell, and SPA
  // navigations can load it late, so a one-time capture would permanently
  // report "No sample cases found" even though they later appeared.
  function collectSamples() {
    const out = [];
    document.querySelectorAll('ul.samples li, .sample-test').forEach((item, i) => {
      const inp = item.querySelector('.input pre, pre.input');
      const o   = item.querySelector('.output pre, pre.output');
      if (inp && o) out.push({ id:i+1, input:inp.textContent, output:o.textContent });
    });
    return out;
  }
  ...
  function submitCode() {
    ...
    if (!csrfToken) {
      // Without the CSRF token the server will reject the POST; rather than
      // silently submitting an invalid form (which navigates to an error page),
      // stop and tell the user how to recover.
      alert('Could not find the page\'s CSRF token — please reload the problem page and try again.');
      return;
    }
  ...
  async function showFilesModal(pidVal, lang) {
    ...
    modal.innerHTML = `<div class="modal-c">
      <div class="modal-h">
        ...
        <button class="mcl" title="Close">&times;</button>
      </div>
      <div class="tabs">
...
async function applyGlobalDarkTheme() {
  ...
  const isSubmissionPath = /(^|\/)submissions?(?:\/|$)/i.test(path) || /^\/problems\/[^/]+\/submit(?:\/|$)/i.test(path);
  document.documentElement.classList.toggle('nzoi-submission-page', isSubmissionPath);
...
:root{
  ...
  --sub-ok-bg:#0f2416;--sub-ok-fg:#7ee787;
  --sub-bad-bg:#2a1214;--sub-bad-fg:#ff9b95;
  --sub-warn-bg:#261d0b;--sub-warn-fg:#e3b341;
  --sub-memory-bg:#221735;--sub-memory-fg:#d2a8ff;
  --syn-kw:#569cd6;--syn-str:#ce9178;--syn-com:#6a9955;--syn-num:#b5cea8;--syn-fn:#dcdcaa;
...
:root[data-theme="light"]{
  ...
  --sub-ok-bg:#dafbe1;--sub-ok-fg:#116329;
  --sub-bad-bg:#ffebe9;--sub-bad-fg:#a40e26;
  --sub-warn-bg:#fff8c5;--sub-warn-fg:#7d4e00;
  --sub-memory-bg:#fbefff;--sub-memory-fg:#8250df;
  --syn-kw:#0451a5;--syn-str:#a31515;--syn-com:#008000;--syn-num:#098658;--syn-fn:#795e26;
...
html.nzoi-submission-page .ui.red.message,
html.nzoi-submission-page .ui.negative.message,
html.nzoi-submission-page .ui.error.message,
html.nzoi-submission-page .ui.red.segment,
html.nzoi-submission-page .ui.inverted.red.segment{
  background:var(--sub-bad-bg)!important;
  border-color:#5f2027!important;
  color:var(--sub-bad-fg)!important;
}
html.nzoi-submission-page .ui.red.message *,
html.nzoi-submission-page .ui.negative.message *,
html.nzoi-submission-page .ui.error.message *,
html.nzoi-submission-page .ui.red.segment *,
html.nzoi-submission-page .ui.inverted.red.segment *{color:inherit!important}
...
/* NZOI submission results: preserve the site's original table styling. */
table.results{border-spacing:0!important;border-collapse:collapse!important;width:100%!important;background:#fff!important;color:#000!important;margin:15px 0!important}
table.results,table.results *{color:#000!important}
.results th{text-align:left!important}
.results td,.results th{padding:2px 20px!important;vertical-align:top!important;border-color:#000!important;background:transparent!important;background-color:transparent!important}
.results tbody.headings{border-width:2px 2px 0!important;border-style:solid solid none!important;border-color:#000 #000 currentcolor!important}
.results tbody.summary{border:2px solid #000!important;font-size:larger!important}
.results tbody th.judgement{text-align:right!important}
.results tbody.compilation,.results tbody.test_set,.results tbody.summary{border-width:2px!important;border-style:solid!important}
.results .compilation.status_success,.results .status_correct{background-color:rgb(96,192,64)!important;border-color:rgb(48,96,32)!important}
.results .status_pending{background-color:rgb(96,96,255)!important;border-color:rgb(48,48,128)!important}
.results .status_cancelled{background-color:rgb(176,176,192)!important;border-color:rgb(85,85,96)!important}
.results .status_partial{background-color:rgb(192,192,0)!important;border-color:rgb(96,96,0)!important}
.results .status_error{background-color:rgb(255,160,0)!important;border-color:rgb(128,80,0)!important}
.results .status_signal,.results .status_runtime{background-color:rgb(160,0,0)!important;border-color:rgb(80,0,0)!important}
.results .status_wrong,.results .status_timeout,.results .status_walltime,.results .status_memory{background-color:rgb(224,48,48)!important;border-color:rgb(104,24,24)!important}
.results tr.test_case td{background-color:rgba(255,255,255,.3)!important}
.results tr.inspect_test>td{background-color:rgba(255,255,255,.6)!important}
.results .compilation td{background-color:rgba(255,255,255,.5)!important}
.results .code{font-family:monospace!important;white-space:pre-wrap!important}
.results tbody.compilation td{padding:10px!important}
.results .test_time,.results .test_memory,.results .judgement{white-space:nowrap!important}
.results .test_name{width:100%!important}
.results .inspect_test table.inspect_meta th:first-of-type{padding-left:2.5px!important}
.results .inspect_test table.inspect_output{width:100%!important;border-spacing:5px!important;table-layout:fixed!important}
.results .inspect_test table.inspect_output th{padding-left:0!important}
.results .inspect_test table.inspect_output th:last-of-type{padding-right:0!important}
.results .inspect_test table.inspect_output td{padding:2px!important;border:1px solid #000!important;overflow:hidden!important;text-overflow:ellipsis!important}
.results .inspect_test table td{background-color:rgba(255,255,255,.3)!important}
.results tr.inspect_test{display:none}
.results tr.test_case:hover+tr.inspect_test,.results tr.inspect_test:hover,.results tr.sample_case+tr.inspect_test:not(.status_correct){display:table-row!important}
.results .output_stats{float:right!important;font-weight:normal!important;font-style:italic!important;padding-left:10px!important}
`);

  const keepGlobalDarkStyleLast = () => {
    const style = document.getElementById('nzoi-global-dark');
    const head = document.head || document.documentElement;
    if (style && head && head.lastElementChild !== style) head.appendChild(style);
  };
  keepGlobalDarkStyleLast();
  setTimeout(keepGlobalDarkStyleLast, 0);
  setTimeout(keepGlobalDarkStyleLast, 500);
  setTimeout(keepGlobalDarkStyleLast, 1500);
  if (document.head) {
    new MutationObserver(keepGlobalDarkStyleLast).observe(document.head, { childList: true });
  }
